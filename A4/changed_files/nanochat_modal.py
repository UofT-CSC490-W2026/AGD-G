...
image = (
    ...
    .add_local_dir(local_path="./nanochat", remote_path="/root/nanochat", ignore='.venv', copy=True)
...
def stage_pretrain(
    ...
    _torchrun(
        ...
        ] + (["--affine"] if affine else []) + (["--nope"] if nope else []),
...
def stage_rl(
    ...
    _torchrun(
        ...
        [
            f"--run={wandb_run}",
            f"--model-tag={model_tag}",
            f"--units-weight={units_weight}",
            f"--consis-weight={consis_weight}",
        ],
        nproc=_N_FINETUNE_GPUS,
    )
...
def eval_rl(
    ...
    print(f"Evaluating saved RL checkpoint: model_tag={model_tag}, model_step={model_step}")
    args = [
        "-i", "rl",
        f"--model-tag={model_tag}",
    ]
    if task_name:
        args.extend(["-a", task_name])
    if model_step is not None:
        args.extend(["-s", str(model_step)])
    if save_predictions:
        args.extend(["--save-predictions", save_predictions])
    _torchrun(
        "scripts.chat_eval",
        args,
        nproc=1,
...
def questions(model1:str, model2:str):
    _setup_cache()
    ...
    questions = [
        ...
        "What is the heaviest metallic element?",
        "Who should I visit to get a cavity filled?",
        "How many wheels does a bicycle have?",
        "Is the sun or the moon bigger?",
        "What year was American independence?",
