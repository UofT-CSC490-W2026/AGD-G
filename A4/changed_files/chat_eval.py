...
def _conversation_assistant_text(conversation):
    """Best-effort conversion of reference assistant content to plain text."""
    try:
...
def run_generative_eval(
    ...
    # Optional per-example output logging for downstream error analysis.
    pred_file = None
    pred_path = None
    if save_predictions:
        pred_path = save_predictions
        if ddp_world_size > 1:
            pred_path = f"{save_predictions}.rank{ddp_rank}.jsonl"
    ...
    for i in range(ddp_rank, num_problems, ddp_world_size):
        ...
        if pred_file is not None:
            ...
            # Convenience fields for GSM8K post-hoc clustering.
            if task_name == "GSM8K":
                gt_response = _conversation_assistant_text(conversation)
                pred_response = completions[0] if completions else ""
                record["gold_answer"] = gsm_extract_answer(gt_response)
                record["pred_answer"] = gsm_extract_answer(pred_response)
                record["gold_response"] = gt_response
                record["pred_response"] = pred_response
    ...
    if pred_file is not None:
        pred_file.close()
        print0(f"Saved predictions: {pred_path}")
...
def run_chat_eval(task_name, model, tokenizer, engine,
                   ...
                   max_problems=None, save_predictions=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--save-predictions', type=str, default=None, help='Optional JSONL path for per-example outputs')
    ...
    for task_name in task_names:
        ...
        if args.save_predictions is not None:
            stem, ext = os.path.splitext(args.save_predictions)
            ext = ext or ".jsonl"
            save_predictions_path = f"{stem}.{task_name}{ext}"
        with autocast_ctx:
            acc = run_chat_eval(
                task_name,
